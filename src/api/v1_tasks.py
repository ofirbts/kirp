...
async def update_node_v1(
    ...
    if updated.get("entity") == "task":
        try:
            ...
            await notify_user(tid, uid, "task_updated", "Task updated", updated.get("title") or "Task", entity_id=node_id, meta={"task_id": node_id})
        except Exception:
            pass
        try:
            ...
            if status == "completed":
                await record_history(tid, updated.get("space_id") or "all", uid, "task_completed", "Task completed: " + tit, tit, source="api", entity_id=node_id)
            else:
                await record_history(tid, updated.get("space_id") or "all", uid, "task_updated", "Task updated", tit, source="api", entity_id=node_id)
        except Exception:
            pass
        ...
        try:
            ...
            await record_history(tid, updated.get("space_id") or "all", uid, "project_updated", "Project updated: " + tit, tit, source="api", entity_id=node_id)
        except Exception:
            pass
...
async def create_task_v1(
    ...
    meta = {"source": "dashboard", "user_id": uid}
    ...
    try:
        from src.core.notifications import notify_user
        await notify_user(tid, uid, "task_created", "Task created", title or "Untitled", space_id=space_id, entity_id=node_id, meta={"task_id": node_id})
    except Exception:
        pass
    try:
        from src.core.history import record_history
        await record_history(tid, space_id, uid, "task_created", "Task created: " + (title or "Untitled"), title or "Untitled", source="api", entity_id=node_id)
    except Exception:
        pass
...
async def create_node_v1(
    ...
    if entity_enum == SchemaEntity.COMMITMENT:
        try:
            from src.core.history import record_history
            await record_history(tid, space_id or ctx.space_id or "all", uid, "commitment_created", "Commitment added", title or "Untitled", source="api", entity_id=node_id)
        except Exception:
            pass
    return {"ok": True, "node": node}
